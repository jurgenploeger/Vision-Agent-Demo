...
listening, thinking, speaking) and a palette of up to three colours.

## Colour

Colours are full **HSV** (hue + saturation + lightness), so any brand colour —
including muted, pastel, or dark tones — renders true rather than being forced
to a fixed vivid version. Each colour is a **swatch**; tapping one opens a
colour picker with a hue wheel + saturation/value square, a hex input, and a row
of preset swatches. The picker is a popover on desktop and a stacked, iOS-style
sheet on mobile. **Shuffle** generates a harmonised palette.

## Other

- Light / dark themes that follow the OS and can be toggled manually.
- A desktop ⇄ mobile preview toggle (the desktop window is resizable).
- iOS-style settings sheet on mobile, revealed by scrolling.
